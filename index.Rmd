...
80's have the most degrees. The '50s and '60s are comparatively lacking. It 
...
x %>% 
  ...
  labs(title = "Decades",
       subtitle = "Most degrees in the '70s and '80s",
       x = "Number of Degrees",
       y = "Decade")
```
...
Among all races, males outnumber females. The only exceptions are the for those
...
of precedence. The first, and so primary, country is used for each entry here. 
...
x %>% 
  ...
  mutate(party = str_replace_all(party, "Independent", "I")) %>% 
  drop_na(party) %>% 
  ggplot(aes(y = party)) +
  geom_bar() +
  scale_y_discrete(limits = rev(c("D", "R", "I", "L", "Green", "Democratic Progressive Party",
                   ...
                   labels = rev(c("Democrat", "Republican", "Independent", "Libretarian", 
                              "Green", "Democratic Progressive Party",
                              "Kuomintang", "National Democratic Party of Tibet", 
                              "Popular Democratic", "Reform"))) +
  labs(title = "Party", 
       x = "Number", 
...
Democrats are nearly equal in gender. Compared to the gender ratio among all the entries, women are overrepresented among the Democrats and underrepresented among the Republicans. 
...
x %>% 
  ...
  geom_bar() +
  labs(title = "Roles of All Phase One Entries", 
       x = "Number", 
       y = "")
